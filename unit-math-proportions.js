  ...
  cards: [
    ...
    /* ───────────────── LESSON 1 ───────────────── */
    {
      ...
      blocks: [
        ...
        { type: 'prose', body: '<p>Two quantities are <b>proportional</b> when one is always the <em>same number</em> times the other. That number is the <b>constant of proportionality</b>, and we call it <b>k</b>. The whole unit is really one equation:</p><blockquote style="font-size:1.3rem; text-align:center; font-weight:700;">y = k · x</blockquote><p>For the tickets, <b>k = 8</b> (dollars per ticket), so cost = 8 × (number of tickets). Every proportional relationship has its own k. Your job this unit: <b>find k</b> from a table, an equation, or a graph — and use it to predict.</p><p>Not everything is proportional. Your age and your height both go up, but not by the same factor every year — so that\'s <em>not</em> proportional. You\'ll learn to tell the difference.</p>' },
